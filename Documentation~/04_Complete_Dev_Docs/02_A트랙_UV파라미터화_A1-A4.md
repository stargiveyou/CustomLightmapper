...
## A4''' — SSAA: 텍셀 내부 슈퍼샘플링 (`TexelMapper.MapSubsamples` / `LightmapSSAA`)

### 문제

위 래스터는 텍셀당 **정중앙 1점**만 복원한다. 그래서 그림자 경계가 텍셀 격자에 양자화되고,
런타임에 UV 확대(magnification)를 거치면 텍셀 크기의 계단으로 보인다.

태양 원반 샘플링(`DirectSamples` + `AngularDiameterDeg`)은 반그림자를 만들지만 이 계단을 항상 없애지는 못한다 —

```
반그림자 폭 ≈ 차폐물까지의 거리 × tan(각지름)
```

차폐물이 1m 떨어져 있고 각지름 2°면 반그림자는 약 3.5cm다. 바닥 텍셀이 20cm면 반그림자가
**텍셀 하나 안에 통째로 들어가** 경계 텍셀 하나가 중간값 하나를 갖는 것으로 끝나고, 계단이 그대로 남는다.

후처리 필터(FXAA 류)로는 못 고친다. 원인이 "서브텍셀 커버리지 정보가 애초에 없음"이라 필터는
정보를 만들지 못하고 오차를 한 텍셀 폭으로 번지게 할 뿐이다. 게다가 아틀라스에서 인접 텍셀은
월드에서 남남일 수 있고(차트 경계), `valid`·노멀 가이드가 없는 필터는 디레이션 링(가짜 값)까지
빨아들여 차트 경계에 rim 을 만든다 — 시임 스티칭에서 이미 겪은 실패 모드다.

### 해법 — 적응형 2패스

```
1패스: 기존대로 텍셀당 1샘플 베이크 → rad[li]
   ↓
엣지 검출: LightmapSSAA.DetectEdges — 상대 휘도차 + 노멀/거리 게이팅 + 1링 확장
   ↓
2패스: 엣지 텍셀만 TexelMapper.MapSubsamples 로 S×S 재래스터
       → 같은 백엔드(BakePoints)로 배치 베이크 → 평균으로 rad[li] 덮어쓰기
```

1패스 값은 엣지 텍셀에서 **버린다**. 짝수 S의 서브샘플 격자에는 중앙점이 없어 재사용하면
가중치가 편향되고, 엣지는 전체의 몇 % 라 버리는 비용이 무시할 수준이다.

### 서브샘플 위치 — 왜 정규격자가 아닌가

```csharp
// Runtime/TexelMapper/TexelMapper.cs — SubOffset
u = (sub + 0.5f) / n;                             // 층화(stratified), n = S*S
float g = sub * 0.6180339887498949f + 0.5f / n;   // 황금비 회전
v = g - Mathf.Floor(g);
```

정규격자 `(sx+0.5)/S` 를 쓰면 **축에 나란한 경계에서 유효 계조가 S 단계밖에 안 나온다**
(S=2면 0/50/100% 셋뿐 — 계단이 그대로 남는다). 두 축 모두 n=S² 단계가 나오도록
u는 층화, v는 황금비 저불일치로 흩는다. RNG 가 아니라 인덱스만으로 정해지는 수열인 이유는
`SunConeDirection`과 같다 — 결정적이고 재현 가능하다.

### 엣지 검출의 게이팅

인스턴스의 루멜 격자에서 인접 텍셀은 **다른 차트일 수 있고**, 그 경계의 값 차이는 에일리어싱이
아니라 정상적인 불연속이다(스티칭이 다룰 몫). 그래서 이웃을 두 조건으로 거른다 —
디노이즈가 쓰는 가이드와 같은 발상이다.

| 게이트 | 기준 | 거르는 대상 |
|---|---|---|
| 노멀 | `dot(n, n') ≥ cos(ssaaEdgeNormalAngleDeg)` | 큐브 모서리·필러 등 각진 면의 정상 불연속 |
| 월드 거리 | `|p - p'| ≤ 4 / texelsPerWorldUnit` | 아틀라스에선 이웃이나 월드에선 먼 다른 차트 |

차이는 **상대값**으로 본다. 라이트맵은 linear HDR 조도라 1을 넘고, 절대 임계값을 쓰면
밝은 영역이 전부 엣지가 되어버린다.

### 파이프라인 불변식

`LumelSubsamples`는 아틀라스 해상도도 `valid` 마스크도 건드리지 않는다 —
부분 커버 텍셀을 새로 켜지 않으므로 Denoise / Seam Stitch / Dilation / ST 매핑은 **전부 그대로**다.
`ssaaMode = Off` 는 기존 경로와 **비트 동일**하다.

| 인스펙터 | 의미 | 권장 |
|---|---|---|
| `ssaaMode` | Off / Adaptive(엣지만) / Full(전 텍셀, S²배) | Adaptive |
| `ssaaFactor` S | 텍셀당 S×S 서브샘플 | 2 (부족하면 3) |
| `ssaaEdgeThreshold` | 이웃과의 상대 휘도차 임계 | 0.05 ~ 0.2 |
| `ssaaEdgeNormalAngleDeg` | 같은 표면 이웃 인정 각도 | 45 |
| `ssaaEdgeDilate` | 엣지 마스크 확장 링 수 | 1 (0이면 계단 한쪽만 매끈해져 오히려 티남) |
| `ssaaDebugMask` | 재샘플 텍셀을 마젠타로 가시화 | 튜닝 시에만 |

완료 로그에 재샘플 비율과 실제 레이 배수가 찍힌다:
`ssaa=Adaptive 2×2(텍셀 4128/121000=3.4%, rays ×1.14)`

**주의**: SSAA는 계단을 없애지만 그림자를 실제보다 **선명하게 만들지는 못한다**.
정보 상한이 텍셀 크기이므로, 넓은 바닥면은 결국 `texelsPerWorldUnit` 을 올려야 한다.
